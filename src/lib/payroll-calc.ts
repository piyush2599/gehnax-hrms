...
export interface EmployeeSalary {
  ...
  pf?: number | null;           // employee PF, monthly (new records)
...
/** Employee PF for a FULL month, derived from the stored salary structure. */
export function fullMonthEmployeePF(sal: EmployeeSalary): number {
  if (sal.pfType === "none") return 0;
  if (sal.pf != null) return round(sal.pf);          // explicitly stored value wins
  // Legacy / unset → statutory 12% of capped basic
  return round(Math.min(sal.basic, PF_WAGE_CEILING) * 0.12);
}

/** TDS for a FULL month. Prefers the stored tds; otherwise unbundles legacy deductions. */
export function fullMonthTDS(sal: EmployeeSalary): number {
  if (sal.tds != null) return round(sal.tds);
  const legacy = sal.deductions || 0;
  if (legacy <= 0) return 0;
  const pf = fullMonthEmployeePF(sal);
  const gross = sal.basic + sal.hra + sal.allowances;
  const esi = gross <= ESI_GROSS_LIMIT ? round(gross * ESI_EMPLOYEE_RATE) : 0;
  return Math.max(0, legacy - pf - esi);
}
...
export function computeSalary(sal: EmployeeSalary, opts: ComputeOptions): SalaryBreakdown {
  ...
  const esi = proGross <= ESI_GROSS_LIMIT ? round(proGross * ESI_EMPLOYEE_RATE) : 0;
